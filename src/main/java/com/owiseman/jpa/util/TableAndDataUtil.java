...
public class TableAndDataUtil implements  TabaleAndDataOperation {
    ...
    public static void processRequest(DSLContext dslContext,
        ...
        switch (operation) {
            ...
            case "select_batch" ->  getInstance().SelectBatchData(dslContext, rootNode);
    ...
    public void alterTable(DSLContext dslContext, JsonNode rootNode) {
        ...
        for (JsonNode column : columns) {
            ...
            switch (operation.toLowerCase()) {
                case "add" -> {
                    if (columnType == null) throw new IllegalArgumentException("Column type is required for add opertion");
                    dslContext.alterTable(tableName).addColumn(columnName, getSqlDataType(columnType)).execute();
                }
                case "modify" -> {
                    if (columnType == null) throw new IllegalArgumentException("Column type is required for modify opertion");
    ...
    public void SelectData(DSLContext dslContext, JsonNode rootNode) {
        ...
        Result<Record> result = dslContext.selectFrom(DSL.table(tableName))
                .where(condition)
                .fetch();
    ...
    @Override
    public void SelectBatchData(DSLContext dslContext, JsonNode rootNode) {
        String tableName = rootNode.get("table").asText();
